...
void Asteroids::OnTimer(int value)
{
    ...
    {
        mSpaceship->Reset();
        mSpaceship->SetInvulnerable(3000); // 3 seconds invulnerable on respawn
...
{
    // Add a border around the edge of the game display
...
void Asteroids::OnLifeGained(int lives_total)
{
    // Just update the lives label, no explosion or respawn
    std::ostringstream msg_stream;
    msg_stream << "Lives: " << lives_total;
    std::string lives_msg = msg_stream.str();
    mLivesLabel->SetText(lives_msg);
}
